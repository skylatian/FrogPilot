...
The devcontainer (`.devcontainer/`) is also available for non-Ubuntu systems.

Python version: 3.11. Dependency management: Poetry (`pyproject.toml` + `poetry.lock`).

**Important**: The stable `FrogPilot` branch strips all dev tools (cabana, replay, joystick, sim, setup scripts — 190 files removed from `tools/`). Our `retrofit-dev` branch is based on `MAKE-PRS-HERE` commit `f5a31a0` (Nov 1st, 2025) which has the **same 0.9.7 codebase and architecture** as the stable release, but with the full dev tooling intact.
...
| Branch | OP Base | Use |
|--------|---------|-----|
| `FrogPilot` (stable) | 0.9.7 | Current release — stripped of dev tools |
| `FrogPilot-Staging` | 0.9.7 | Beta testing |
| `FrogPilot-Development` | 0.10.3 | Dev-only, hardcoded to developer's device |
| `MAKE-PRS-HERE` | Mixed | PR workspace, auto-syncs with stable — tip is 0.10.3, but older commits (pre-Dec 2025) are 0.9.7 |

**Stable vs MAKE-PRS-HERE**: The stable `FrogPilot` branch deletes ~190 dev tool files (cabana, replay, joystick, sim, mac_setup.sh, etc.) from the release. MAKE-PRS-HERE retains them. Commits on MAKE-PRS-HERE before December 2025 share the same 0.9.7 architecture as stable — identical car code, imports, and structure. The 0.10.3 rebase happened later in MAKE-PRS-HERE history.

**Our branch (`retrofit-dev`)**: Based on MAKE-PRS-HERE commit `f5a31a0` (Nov 1st, 2025 Update) — same code as the stable release, full dev tools present.

A migration from 0.9.7 → 0.10.3 is in progress. When it ships, car code moves from `selfdrive/car/toyota/` to `opendbc_repo/opendbc/car/toyota/` and imports change from `openpilot.selfdrive.car` to `opendbc.car`. The actual steering/CAN logic is structurally identical — porting is a ~2 hour mechanical refactor (import paths + constructor signatures).
